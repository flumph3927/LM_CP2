...
import turtle, fractal_generator.fractals, fractal_generator.screen_background
...
def main():
    ...
    while True:
        ...
        if choice=='1':
            ...
            scrn.bgcolor(fractal_generator.screen_background.bckgrnd())
            ...
            while depth not in [str(x+1) for x in range(6)]:
                ...
                depth=input('File depth(1-6):')
            turtle.tracer(0,0)
            fractal_generator.fractals.generate(t,int(depth),200)
            turtle.update()
            if input('S to save as image: ').lower()=='s':
                fractal_generator.screen_background.save_image(scrn)
        #repeat creat fractal one but for fractal 2
            ...
            while depth not in [str(x+1) for x in range(5)]:
                ...
                depth=input('File depth(1-6):')
            turtle.tracer(0,0)
            fractal_generator.fractals.other_fractal(t,int(depth)+1,200)
            turtle.update()
            if input('S to save as image: ').lower()=='s':
                fractal_generator.screen_background.save_image(scrn)
